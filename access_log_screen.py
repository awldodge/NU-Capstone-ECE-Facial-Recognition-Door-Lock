...
def show_screen(root, go_back_callback, go_to_settings_log_callback):
    ...
    # Title
    title = tk.Label(frame, text="Access Attempt Logs (Last 2 Weeks)", font=("Helvetica", 20, "bold"),
                     fg="white", bg="#000000")
    title.pack(pady=10)
    ...
    def refresh_display():
        ...
        draw_static_headers()
        render_visible_rows()
        title.config(text="Access Attempt Logs (Last 6 Months)" if showing_six_months[0]
                     else "Access Attempt Logs (Last 2 Weeks)")
    ...
    tk.Button(inner_frame, text="Return to View Logs Screen", command=go_back_callback,
              font=("Helvetica", 14), width=24, bg="#66cc66", fg="white",
              activebackground="#99e699").pack(side="left", padx=12)

    def toggle_log_range():
        showing_six_months[0] = not showing_six_months[0]
        refresh_display()

    tk.Button(inner_frame, text="Toggle 2wks / 6mos", command=toggle_log_range,
              font=("Helvetica", 14), width=24, bg="#ffcc00", fg="black",
              activebackground="#ffe066").pack(side="left", padx=12)